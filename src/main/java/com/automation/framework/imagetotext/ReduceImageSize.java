...
public class ReduceImageSize {
	public static  void main(String args[]) throws IOException {
		reduceSize("reduce image to size 10KB /home/liveuser/Pictures/Screenshots/onetwo.png");
	}
	public static String reduceSize(String query) throws IOException {
		...
		inputFilePath = queryStr[1].trim();
		String sizeDimension = queryStr[0];

		// Check for keyword "to pixels" in the query to handle pixel resizing
		if (sizeDimension.contains("x")) {
...
//			inputFilePath = parts[1].trim(); // The part before "to pixels" is the input file path
			String dimensions = sizeDimension.trim(); // The part after "to pixels" is the dimension (e.g., 1080x1080)
		...
		else if (sizeDimension.toUpperCase().contains("KB") || sizeDimension.toUpperCase().contains("MB")) {
			// Split using "to size"
//			String[] str = query.split("to size");
//			String strAfterPixels = str[1].trim();
//			String parts[] = strAfterPixels.split(" ");
//			inputFilePath = parts[1].trim(); // The part before "to size" is the input file path
			targetSize = sizeDimension.toUpperCase().trim(); // The part after "to size" is the target size (e.g., 40KB)
		}
		...
		if (!inputFilePath.isEmpty()) {
			...
			outputFilePath = inputFile.getParent() + File.separator + fileBaseName + "_output." + fileExtension;
			if (inputFilePath.contains(".png")) {
				inputFilePath = ImageConversion.convertPngToJpg(inputFilePath,
						inputFile.getParent() + File.separator + fileBaseName + ".jpg");
				outputFilePath = inputFile.getParent() + File.separator + fileBaseName + "_output.jpg";
				fileExtension = "jpg";
				flag = true;
			}
		...
		try {
			...
			if (flag) {
				...
				ImageConversion.convertJpgToPngSpecificSize(outputFilePath,
						inputFile.getParent() + File.separator + fileBaseName + "_output.png", scaleValue);
				inputFile.delete();
			}
